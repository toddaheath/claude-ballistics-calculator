...
public class TrajectoryCalculator
{
    ...
    private double FindZeroAngle(Cartridge cartridge, double zeroRangeFt, double sightHeightInches)
    {
        ...
        // The bullet starts sightHeight below the line of sight (bore is below scope)
        // We need to find the angle where height relative to LOS = 0 at zero range
        double lowAngle = 0.0;
        ...
        for (int i = 0; i < MaxZeroIterations; i++)
        {
            ...
            double heightAtZero = SimulateHeightAtRange(cartridge, midAngle, zeroRangeFt, sightHeightFt);
    ...
    private double SimulateHeightAtRange(Cartridge cartridge, double angleRad,
        double rangeFt, double sightHeightFt)
    {
        ...
        double ballisticCoefficient)
    {
        ...
        double dragDecel = BallisticConstants.AirDensity * velocity * cd /
                          (2.0 * bcFps);

        // Drag acts opposite to velocity direction
        double ax = -dragDecel * vx;
        double ay = -dragDecel * vy - BallisticConstants.Gravity;
